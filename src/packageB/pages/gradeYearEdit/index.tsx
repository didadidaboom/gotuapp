...
interface GradeYearPage {
  props: PageStateProps;
}

@inject("store")
@observer
class GradeYearPage extends Component {
  state={
    selected:null
  }
  ...
  toNextPage = () => {
    Taro.navigateTo({
      url: "/pages/schoolSelectionPage/index",
    });
  };
  ...
  selectedGrade = (selected) => {
    ...
    const { registerStore, profileStore } = this.props.store;
    let res =  registerStore.submitGrade(selected)
    this.setState({
      selected
    })
    if(res){
      let grade = registerStore.gradeMapToParam(selected)
      profileStore.profile.gradeClass = grade.gradeClass;
      profileStore.profile.gradeYear = grade.gradeYear;
      this.gobackClick()
    }
  };
